...
final class Validators {
    ...
    static void checkLeaderboardRequest(LeaderboardRequest req) throws ApiValidationException {
        if (isInvalid(req.getGameId())) {
            throw new ApiValidationException(GENERIC_ELEMENT_QUERY_FAILURE, "Game Id is null or invalid!");
        } else if (Objects.isNull(req.getTimeRange())) {
            throw new ApiValidationException(GENERIC_ELEMENT_QUERY_FAILURE, "TimeRange must be one of valid values!");
        } else if (req.getTimeRange() != TimeScope.ALL && StringUtils.isBlank(req.getTime())) {
            throw new ApiValidationException(GENERIC_ELEMENT_QUERY_FAILURE, "Time parameter mandatory when time range is specified!");
        }
    }

    static void checkRankingRequest(UserRankingRequest req) throws ApiValidationException {
        if (isInvalid(req.getGameId())) {
            throw new ApiValidationException(GENERIC_ELEMENT_QUERY_FAILURE, "Game Id is null or invalid!");
        } else if (isInvalid(req.getUserId())) {
            throw new ApiValidationException(GENERIC_ELEMENT_QUERY_FAILURE, "User Id is null or invalid!");
        } else if (Objects.isNull(req.getDate())) {
            throw new ApiValidationException(GENERIC_ELEMENT_QUERY_FAILURE, "Date parameter is mandatory!");
        }
    }
